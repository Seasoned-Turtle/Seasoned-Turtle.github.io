// 生日提醒数据与逻辑 (合并 birthday1.JS 和 birthday2.JS)
(function() {
        const BIRTHDAYS_DATA = [
                ...
                { name: 'ヴリティカ・ バラクリシュナン（《Heaven Burns Red》）', birthday: '01-26' },
        ...
        function renderBirthdays() {
                ...
                for (const item of BIRTHDAYS_DATA) {
                        const [month, day] = item.birthday.split('-').map(Number);
                        ...
                        const diffDays = Math.ceil((birthdayDate - today) / (1000 * 3600 * 24));
                ...
                if (upcomingList.length === 0) {
                        container.innerHTML = '<div class="birthday-item">✨ 未来一个月内没有人过生日，放松一下吧～</div>';
                        return;
                }
                ...
                for (const b of upcomingList) {
                        ...
                        if (b.diffDays === 0) {
                                ...
                                div.innerHTML = `🎉 ${b.name} 今天过生日！ 🎂 祝TA生日快乐～`;
                        } else {
                                div.innerHTML = `🎈 ${b.name} · 生日 ${b.birthday} &nbsp;&nbsp;🌟 还有 ${b.diffDays} 天`;
                        }
